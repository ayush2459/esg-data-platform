...
const SC = {1:'#ef4444',2:'#3b82f6',3:'#a78bfa'}
...
function Btn({onClick,icon:Icon,label,color,disabled}){
  return <button onClick={onClick} disabled={disabled}
    style={{display:'flex',alignItems:'center',gap:5,padding:'5px 10px',borderRadius:6,border:`1px solid ${color}30`,background:`${color}12`,color,fontSize:12,fontWeight:500,cursor:disabled?'not-allowed':'pointer',opacity:disabled?0.4:1}}
...
function Row({record,selected,onSelect,onAction}){
  const [exp,setExp]=useState(false)
  const [note,setNote]=useState('')
  const [acting,setActing]=useState(false)
  const isLocked=record.status==='LOCKED'
  const doAction=async(action)=>{setActing(true);await onAction(record.id,action,note);setActing(false);setExp(false)}
  return(
    <>
      <tr style={{borderBottom:'1px solid var(--border)',background:selected?'rgba(59,130,246,0.05)':exp?'var(--bg3)':'transparent'}}>
        <td style={{padding:'12px 10px'}}>
          <input type="checkbox" checked={selected} onChange={()=>onSelect(record.id)} onClick={e=>e.stopPropagation()} style={{cursor:'pointer',accentColor:'var(--accent)'}}/>
        </td>
        <td style={{padding:'12px 10px'}} onClick={()=>setExp(!exp)}>
          <span style={{fontSize:11,fontWeight:600,color:SC[record.scope],background:`${SC[record.scope]}15`,padding:'2px 7px',borderRadius:4}}>S{record.scope}</span>
        </td>
        <td style={{padding:'12px 10px'}} onClick={()=>setExp(!exp)}>
        ...
        <td style={{padding:'12px 10px',fontFamily:'monospace',fontSize:12}} onClick={()=>setExp(!exp)}>
          {parseFloat(record.activity_value).toLocaleString(undefined,{maximumFractionDigits:2})} {record.activity_unit}
        </td>
        <td style={{padding:'12px 10px',fontSize:12,color:'var(--text2)'}} onClick={()=>setExp(!exp)}>
          <div>{record.facility||'—'}</div>
          <div style={{color:'var(--text3)',fontSize:11}}>{record.period_start}</div>
        </td>
        <td style={{padding:'12px 10px'}} onClick={()=>setExp(!exp)}><Badge status={record.status}/></td>
        <td style={{padding:'12px 10px'}} onClick={()=>setExp(!exp)}>
          <ChevronDown size={14} color="var(--text3)" style={{transform:exp?'rotate(180deg)':'rotate(0)',transition:'transform 0.2s'}}/>
        </td>
      </tr>
      {exp&&(
        <tr style={{background:'var(--bg3)'}}>
          <td colSpan={7} style={{padding:'16px 20px',borderBottom:'1px solid var(--border)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:14}}>
              {[
                ['Original',`${parseFloat(record.original_value).toLocaleString()} ${record.original_unit}`],
                ...
                <div key={k}>
                  ...
                  <div style={{fontSize:12,fontFamily:'monospace'}}>{v}</div>
                </div>
              ))}
            </div>
            {!isLocked?(
              <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                ...
                <Btn onClick={()=>doAction('APPROVE')} icon={CheckCircle} label="Approve" color="#22c55e" disabled={acting||record.status==='APPROVED'}/>
                <Btn onClick={()=>doAction('REJECT')} icon={XCircle} label="Reject" color="#9ca3af" disabled={acting||record.status==='REJECTED'}/>
                <Btn onClick={()=>doAction('FLAG')} icon={Flag} label="Flag" color="#ef4444" disabled={acting||record.status==='FLAGGED'}/>
                {record.status==='APPROVED'&&<Btn onClick={()=>doAction('LOCK')} icon={Lock} label="Lock for Audit" color="#a78bfa" disabled={acting}/>}
              </div>
            ):(
              <div style={{fontSize:12,color:'var(--text3)',display:'flex',alignItems:'center',gap:6}}><Lock size={12}/> Locked for audit.</div>
            )}
          </td>
        </tr>
      )}
  ...
  return(
    <div style={{padding:32}}>
      <div style={{marginBottom:24}}>
        ...
        <p style={{color:'var(--text2)',fontSize:13}}>Review, approve, flag, and lock records before audit</p>
      </div>
      <div style={{display:'flex',gap:8,marginBottom:16,alignItems:'center',flexWrap:'wrap'}}>
        ...
        {(filters.status||filters.scope||filters.category)&&(
          <button onClick={()=>setFilters({status:'',scope:'',category:''})} style={{padding:'6px 10px',background:'none',border:'1px solid var(--border)',borderRadius:7,color:'var(--text2)',fontSize:12,cursor:'pointer'}}>Clear</button>
        )}
      ...
      {selected.size>0&&(
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,padding:'10px 14px',background:'rgba(59,130,246,0.08)',border:'1px solid rgba(59,130,246,0.2)',borderRadius:9,flexWrap:'wrap'}}>
          ...
          <input value={bulkNote} onChange={e=>setBulkNote(e.target.value)} placeholder="Add note..."
            style={{padding:'5px 9px',background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:6,color:'var(--text)',fontSize:12,outline:'none'}}/>
          <Btn onClick={()=>handleBulk('APPROVE')} icon={CheckCircle} label="Approve all" color="#22c55e" disabled={bulkActing}/>
          <Btn onClick={()=>handleBulk('REJECT')} icon={XCircle} label="Reject all" color="#9ca3af" disabled={bulkActing}/>
          <Btn onClick={()=>handleBulk('FLAG')} icon={Flag} label="Flag all" color="#ef4444" disabled={bulkActing}/>
      ...
      <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:12,overflow:'hidden'}}>
        ...
        :data.length===0?<div style={{padding:40,textAlign:'center',color:'var(--text3)'}}>No records found.</div>
        :<div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead>
              <tr style={{borderBottom:'1px solid var(--border)'}}>
                <th style={{padding:'10px 10px',textAlign:'left'}}>
                  <input type="checkbox" checked={selected.size===data.length&&data.length>0} onChange={toggleAll} style={{cursor:'pointer',accentColor:'var(--accent)'}}/>
                </th>
            ...
            <tbody>
              {data.map(r=><Row key={r.id} record={r} selected={selected.has(r.id)} onSelect={toggleSelect} onAction={handleAction}/>)}
            </tbody>
          </table>
        </div>}
      </div>
