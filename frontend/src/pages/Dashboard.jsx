  ...
  return (
    <div style={{padding:32}}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={scopeData} barSize={32}>
              ...
              <Bar dataKey="value" radius={[4,4,0,0]}>
                {scopeData.map((e,i)=><Cell key={i} fill={e.color}/>)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:12,padding:20}}>
        ...
        {recentRuns.length===0
          ? <div style={{color:'var(--text3)',fontSize:13}}>No runs yet.</div>
          : recentRuns.slice(0,6).map(run=>(
            <div key={run.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 0',borderBottom:'1px solid var(--border)'}}>
              ...
              <span style={{fontSize:11,fontWeight:500,padding:'3px 8px',borderRadius:4,background:run.status==='DONE'?'rgba(34,197,94,0.12)':'rgba(239,68,68,0.12)',color:run.status==='DONE'?'var(--green)':'var(--red)'}}>
                {run.status}
              </span>
            </div>
          ))
